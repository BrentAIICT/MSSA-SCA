...
1. Create a job trigger object and store it in `$trigger1`. Configure the trigger so the job will:
    ...
    - Run once in two minutes. Use **Get-Date** and a method of the resulting **DateTime** object to calculate two minutes from now.
    ...
    ```
    $Trigger1 = New-JobTrigger -Once -At (Get-Date).AddMinutes(2) 
    ```
    </Strong></details> 

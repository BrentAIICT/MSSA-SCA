...
- On **LON-CL1**, ensure that you're signed in as **Adatum\\Administrator** and determine the answers to the following questions:
  ...
  - What command would you run to resolve a DNS name? <details><summary>Click to see the answer</summary><Strong> 
  ``` 
    Get-Command *resolve*  
  ```
  </Strong></details>
  - What command would you run to make changes to a network adapter? After finding such a command, what parameter would you use to change its Media Access Control (MAC) address (on adapters that support changes to their MAC address)?<details><summary>Click to see the answer</summary><Strong> 
    ``` 
    Get-Command *adapter*
   ```
    </Strong></details>
  ...
  - What command would let you enable a previously disabled scheduled task?<details><summary>Click to see the answer</summary><Strong> 
  ``` 
  Get-Command *sched*  
  ```
  </Strong></details>
  - What command would let you block access to a file share by a particular user?<details><summary>Click to see the answer</summary><Strong> 
  ``` 
    Get-Command -Verb Block  
  ```
  </Strong></details>
  - What command would you run to clear your computer’s local **BranchCache** cache?<details><summary>Click to see the answer</summary><Strong> 
  ``` 
  Get-Command *branch* 
  ```
  </Strong></details>
  - What command would you run to display a list of Windows Firewall rules? What parameter of that command would display only enabled rules?<details><summary>Click to see the answer</summary><Strong> 
  ``` 

...
1. In the Windows PowerShell console, enter the following command, and then select Enter: <details><summary>Click to see the answer</summary><Strong> ```ps Start-Transcript C:\DayOne.txt ```</Strong></details> 

> **Note:** You've now started a transcript of your Windows PowerShell session. It'll save all commands that you enter and the command output to a text file until you run **Stop‑Transcript** or close the Windows PowerShell window. You can review the transcript's contents at any time by opening **C:\DayOne.txt**. <details><summary>Click to see the answer</summary><Strong> ``` Stop-Transcript ```</Strong></details>
...
- On **LON-CL1**, ensure that you're signed in as **Adatum\\Administrator** and determine the answers to the following questions:
  ...
  - What command would you run to resolve a DNS name? <details><summary>Click to see the answer</summary><Strong> ``` Get-Command *resolve*  ```</Strong></details>
  - What command would you run to make changes to a network adapter? After finding such a command, what parameter would you use to change its Media Access Control (MAC) address (on adapters that support changes to their MAC address)?<details><summary>Click to see the answer</summary><Strong> ``` Get-Command *adapter*
   ```</Strong></details><details><summary>Click to see the answer</summary><Strong> ``` Get-Help Get-NetAdapter ```</Strong></details> 
  
  - What command would let you enable a previously disabled scheduled task?<details><summary>Click to see the answer</summary><Strong> ``` Get-Command *sched*  ```</Strong></details>
  - What command would let you block access to a file share by a particular user?<details><summary>Click to see the answer</summary><Strong> ``` Get-Command -Verb Block  ```</Strong></details>
  - What command would you run to clear your computer’s local **BranchCache** cache?<details><summary>Click to see the answer</summary><Strong> ``` Get-Command *branch* ```</Strong></details>
  - What command would you run to display a list of Windows Firewall rules? What parameter of that command would display only enabled rules?<details><summary>Click to see the answer</summary><Strong> ``` Get-Command *firewall* 
Get-Help Get-NetFirewallRule -Full ```</Strong></details>
  - What command would you run to display a list of all locally bound IP addresses?<details><summary>Click to see the answer</summary><Strong> ``` Get-Command *address*  ```</Strong></details>
  - What command would you run to suspend an active print job in a print queue?<details><summary>Click to see the answer</summary><Strong> ``` Get-Command -Verb suspend ```</Strong></details>
  - What native Windows PowerShell command would you run to review the content of a text file?<details><summary>Click to see the answer</summary><Strong> ``` Get-Command -Noun *content*  ```</Strong></details>
...
2. Display a list of enabled Windows Firewall rules.<details><summary>Click to see the answer</summary><Strong> ``` Get-NetFirewallRule -Enabled True ```</Strong></details>

3. Display a list of all local IPv4 addresses.<details><summary>Click to see the answer</summary><Strong> ``` Get-NetIPAddress -AddressFamily IPv4 ```</Strong></details>
...
4. Set the startup type of the BITS service to **Automatic**:
   ...
   b.   Locate the Background Intelligence Transfer Service (BITS) and note its startup type setting prior to and after changing the startup type in Windows PowerShell.<details><summary>Click to see the answer</summary><Strong> ``` Set-Service -Name BITS -StartupType Automatic   ```</Strong></details>

5. Test the network connection to **LON-DC1**. Your command should return only a True or False value, without any other output.<details><summary>Click to see the answer</summary><Strong> ``` Test-Connection -ComputerName LON-DC1 -Quiet ```</Strong></details>

6. Display the newest 10 entries from the local Security event log.<details><summary>Click to see the answer</summary><Strong> ``` Get-EventLog -LogName Security -Newest 10  ```</Strong></details>
...
- Ensure that you'll still signed in to **LON-CL1** as **Adatum\\Administrator** from the previous exercise, and answer the following questions:
  ...
  - What comparison operator does Windows PowerShell use for wildcard string comparisons?<details><summary>Click to see the answer and are Windows PowerShell comparison operators typically case-sensitive?</summary><Strong> ``` Get-Help about_*comparison* ```</Strong></details>
  - How would you use `$Env` to display the **COMPUTERNAME** environment variable?<details><summary>Click to see the answer</summary><Strong> ``` Get-Help About_*env* ```</Strong></details>
  - What external command could you use to create a self-signed digital certificate that's usable for signing Windows PowerShell scripts?<details><summary>Click to see the answer</summary><Strong> ``` Get-Help about_signing  ```</Strong></details>

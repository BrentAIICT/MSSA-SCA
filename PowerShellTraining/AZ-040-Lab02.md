...
- From **LON-CL1**, use Windows PowerShell to create a new OU named **London**.<details><summary>Click to see the answer</summary><Strong> ``` New-ADOrganizationalUnit -Name London ```</Strong></details>

### Task 2: Create a group for branch office administrators

- In the **PowerShell** console, create the **London Admins** global security group.<details><summary>Click to see the answer</summary><Strong> ``` New-ADGroup "London Admins" -GroupScope Global ```</Strong></details>

### Task 3: Create a user and computer account for the branch office

1. In the **PowerShell** console, create a user account for the user **Ty Carlson**.<details><summary>Click to see the answer</summary><Strong> ``` New-ADUser -Name Ty -DisplayName "Ty Carlson"  ```</Strong></details>
2. Add the user to the **London Admins** group.<details><summary>Click to see the answer</summary><Strong> ``` Add-ADGroupMember "London Admins" -Members Ty ```</Strong></details>
3. Create a computer account for the **LON-CL2** computer.<details><summary>Click to see the answer</summary><Strong> ``` New-ADComputer LON-CL2 ```</Strong></details>
...
- Use Windows PowerShell to move the following group, user, and computer accounts to the London OU:
  ...
  - **London Admins**
  - **Ty Carlson**
  - **LON-CL2**<details><summary>Click to see the answer</summary><Strong> ``` Move-ADObject -Identity "CN=London Admins,CN=Users,DC=Adatum,DC=com" -TargetPath "OU=London,DC=Adatum,DC=com" ```</Strong></details><details><summary>Click to see the answer</summary><Strong> ``` Move-ADObject -Identity "CN=Ty,CN=Users,DC=Adatum,DC=com" -TargetPath "OU=London,DC=Adatum,DC=com" ```</Strong></details><details><summary>Click to see the answer</summary><Strong> ``` Move-ADObject -Identity "CN=LON-CL2,CN=Computers,DC=Adatum,DC=com" -TargetPath "OU=London,DC=Adatum,DC=com" ```</Strong></details>
...
3. Test the connection to **LON-DC1** and note the speed of the test.<details><summary>Click to see the answer</summary><Strong> ``` Test-Connection LON-DC1 ```</Strong></details>
4. Review the network configuration for **LON-SVR1**.<details><summary>Click to see the answer</summary><Strong> ``` Get-NetIPConfiguration ```</Strong></details>
5. Note the IP address, default gateway, and DNS server.

### Task 2: Change the server IP address

- Use Windows PowerShell to change the IP address for the **Ethernet** network interface to **172.16.0.15/16**.<details><summary>Click to see the answer</summary><Strong> ``` New-NetIPAddress -InterfaceAlias Ethernet -IPAddress 172.16.0.15 -PrefixLength 16 ```</Strong></details><details><summary>Click to see the answer</summary><Strong> ``` Remove-NetIPAddress -InterfaceAlias Ethernet -IPAddress 172.16.0.11 ```</Strong></details>


### Task 3: Change the DNS settings and default gateway for the server

1. Change the DNS settings of the **Ethernet** network interface to point at **172.16.0.12**.<details><summary>Click to see the answer</summary><Strong> ``` Set-DnsClientServerAddress -InterfaceAlias Ethernet -ServerAddress 172.16.0.12 ```</Strong></details>
2. Change the default gateway for the **Ethernet** network interface to **172.16.0.2**.<details><summary>Click to see the answer</summary><Strong> ``` Remove-NetRoute -InterfaceAlias Ethernet -DestinationPrefix 0.0.0.0/0 -Confirm:$false ```</Strong></details><details><summary>Click to see the answer</summary><Strong> ``` New-NetRoute -InterfaceAlias Ethernet -DestinationPrefix 0.0.0.0/0 -NextHop 172.16.0.2 ```</Strong></details>


### Task 4: Verify and test the changes

1. On **LON-SVR1**, verify the changes to the network configuration.<details><summary>Click to see the answer</summary><Strong> ``` Get-NetIPConfiguration ```</Strong></details>
2. Test the connection to **LON-DC1**, and then note the difference in the test speed.<details><summary>Click to see the answer</summary><Strong> ``` Test-Connection LON-DC1 ```</Strong></details>
...
- On **LON-SVR1**, use Windows PowerShell to install the Web server (IIS) role on **LON-SVR1**.<details><summary>Click to see the answer</summary><Strong> ``` Install-WindowsFeature Web-Server ```</Strong></details>

### Task 2: Create a folder on the server for the website files

- On **LON-SVR1**, use PowerShell to create a folder named **London** under **C:\inetpub\wwwroot** for the website files.<details><summary>Click to see the answer</summary><Strong> ``` New-Item C:\inetpub\wwwroot\London -Type directory ```</Strong></details>
...
1. Open the website in Internet Explorer by using the IP address and port **8080**, and then verify that the site is using the provided settings. Internet Explorer will display an error message that a document hasn't been configured for the URL. The error message details give the physical path of the site, which should be **C:\\inetpub\\wwwroot\\london**.<details><summary>Click to see the answer</summary><Strong> ``` http://172.16.0.15:8080 ```</Strong></details>

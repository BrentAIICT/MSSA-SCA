...
1. Select **Start**, and then enter **ISE**.
...
1. To create a new site, in the console, enter the following command, and then select Enter:
   ...
   New-SPOSite -Url https://$($SPODomainForNewSite)/sites/Sales -Owner noreen@$($AzureADDomain.Name) -StorageQuota 256 -Template EHS#1
   ```
   </Strong></details> 
